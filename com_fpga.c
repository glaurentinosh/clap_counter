	...
	int main(void)
		{
			...
			char  ComPortName[] = "COM4";		   // Nome da Porta Serial,
			BOOL  Status;                          // Status of the various operations
			DWORD dwEventMask;                     // Event mask to trigger
			char  TempChar;                        // Char temporario
			...
			if (hComm == INVALID_HANDLE_VALUE)
				printf("\n -x-x- Erro! A Porta %s nao pode ser aberta...\n", ComPortName);
			else
			...
			if (Status == FALSE)
				{
					printf("\n -x-x-x-x-x- Error! in Setting DCB Structure");
				}
			...
			if (SetCommTimeouts(hComm, &timeouts) == FALSE)
				printf("\n\n -x-x-x-x-x- Error! in Setting Time Outs");
			else
				printf("\n\n -x-x-x-x-x- Setting Serial Port Timeouts Successfull");
			...
			if (Status == FALSE)
				printf("\n\n -x-x-x-x-x- Error! in Setting CommMask");
			else
				printf("\n\n -x-x-x-x-x- Setting CommMask successfull");
				...
				read = 0;
					while (1){
						...
						if (read != TempChar){
							...
							if (TempChar == '1'){
								write = '0' + (write + 1) % 16;
								Status = WriteFile(hComm, &write, sizeof(write), &NoBytesRead, NULL);
								printf("\n |     Som!       | %d", count);
								printf("\n +=========================================================+\n\n");
								//sleep(2);
							}
